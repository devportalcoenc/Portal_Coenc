...
function f_MostrarPublicacoes(){
	...
	for (i=0; i<results.length; i++){
		...
		c = [];
		for (y=0; y<4; y++){c[y]=r.insertCell(y); c[y].setAttribute('class', '');}
		...
		c[1].innerHTML = results[i].categoria;
		c[2].innerHTML = data;
		c[3].setAttribute('class', 'text-center');
		c[3].innerHTML ='<img style="width: 20px; height:20px; cursor:pointer;margin-right:20px" src="img/edit.png" onclick=f_EditarPublicacao('+results[i].idpublicacao+')><img style="width: 20px; height:20px;cursor:pointer" src="img/delete.png" onclick=f_ExcluirPublicacao('+results[i].idpublicacao+')>';
	}
}
